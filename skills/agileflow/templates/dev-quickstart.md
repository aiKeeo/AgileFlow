...
**连续做 / 全部开发 / yes_all** = **先展开 TodoWrite**，再按清单逐条做（每条含可运行闸门）；**≠** 摘要 / 空壳 / 合并多 T / **从不编译启动** / **跳过 TodoWrite** / **扁平 todo** / **薄写构思**。快速模式同样遵守：无①禁写码、禁合并 T、禁跳过 `dev-step1-literal` / 可运行闸门；**sol/dev 与严谨同质**。

| 用户说 / 模式 | 主 Agent 必须 | 禁止 |
|---------------|---------------|------|
| 全部开发 / 直接全开发 | **先**确认三段式 todo → TodoWrite 为每个 T 建① → 串行 ①→②→可运行闸门→③；`AF_FLOW=fast` 且 ≥2 无冲突 T → **另推**并行启动卡 | 无卡启 Task；扁平 `- [ ] T-001`；无①就写码；跳过可运行闸门；薄写步骤 |
| 并行 / 要快 / 快点 / 加速（显式）或 **快速模式推并发** | 先读 [parallel-orchestration](../phases/parallel-orchestration.md)；每 T **同质**合规①；②后仍过可运行闸门 | 无 dev 就让 Subagent Write；用减文档换速度；Subagent 跳过 C |
| 给用户看 / MVP 演示 | 存在端编译+启动+MVP 主路径冒烟全过 | 「代码齐了」就直接交给用户 |

**未开并行启动卡 → 禁止启用「串行约束」并行例外。** 快速要加速 → 推并发，不是薄①。对照 → [fast-means-parallel](../examples/fast-means-parallel.md)。
...
| **步骤可执行** | **原子步骤表**（每 `#### S1…` 一个步骤 + 8 字段规格表 `| 字段 | 内容 |`：执行角色/触发条件/输入数据/处理逻辑含 if-else/调用依赖 `Service.method(params)`/异常处理 错误码+回滚/输出数据/状态变更）或精简 `####` + **涉及改动**/**改**（含 `` `method` ``）；标准≥2 / 完整≥3 / 精简≥1 步（legacy：`AF_TEMPLATE=no` 仍可用 用户/系统/改；旧 5 列流程表兼容但不再推荐）。**完整档 / AI 自主复杂链路须原子步骤表** | 按 exemplar / [dev-reuse-examples](../examples/dev-reuse-examples.md) 重写 |
| **链 sol** | BE 链 API-xxx；FE 链 UI-xxx（+UID）；dev **禁**字段映射表 | 补链 |
| **非事后补写** | 非事后补写；无 `顺序：⚠️` | 标 ⚠️；重写 |
| **反假标题** | 纯文本段名无 `##` → 不过；文档过短且无步骤（原子步骤表或 ####）→ 不过 | 按 exemplar 重写整份 |
...
步骤：**原子步骤表** S1…（每 `####` 一个步骤 + 8 字段规格表：执行角色/触发条件/输入数据/处理逻辑含 if-else/调用依赖 `Service.method(params)`/异常处理 错误码+回滚/输出数据/状态变更）优先；精简可用 `####` + `- **改**：` `` `Class.method()` ``。旧 5 列流程表兼容但不再推荐。禁旧标题 `## 一、目标` / `## 五、可执行方案`。  
例子 → [dev-reuse-examples](../examples/dev-reuse-examples.md) · exemplar-BE/FE。
...
```
## 摘要          ← 本T/做/不做/上游/AC（标准+五 bullet；做含接法）
## 步骤          ← 原子步骤表 S1…（完整/标准优先）或 ####+改（精简）；AC 只在摘要
## 结果          ← ③ 可运行证据
```
