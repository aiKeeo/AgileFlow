...
| 检查项 | 标准 |
|--------|------|
| **读范例** | 已 Read exemplar-BE 或 exemplar-FE |
| **串行约束** | **串行**：只 1 个 T；无其他「①勾完②未完」。**显式并行**：批量构思 允许多个①已勾②未开；写码执行批 允许多 T 同时②（1 Task=1 T） |
| **文件存在** | `atlas/dev/T-xxx-*.md` 已落盘 |
| **段标题** | 按档位：`## 范围`/`## 做法`/`## 结果`（标准+ `## 契约`/`## AC`） |
| **做法可执行** | `####` 步骤 + Class.method/pages；FE 契约含 `### 布局`（ASCII）+ `### 映射` |
| **非事后补写** | 非 ⚠️ 事后补写 / 非摘要 |
| **字面量** | 完整档：`--gate dev-step1-literal` exit 0；模板见 [dev-rationale](dev-rationale.md) |
...
| **① 已勾** | 该 T 构思完成 |
| **单任务** | Write ⊆ 当前「## 做法」步骤 |
| **写法锚点** | 已 Read 本端资产索引（[code-conventions](code-conventions.md)） |
| **复用** | 契约里写「复用」的路径须真用 |
