...
**入口**：`/af-mod`（`/af-model` 同义）或 flow 到建模步。阶段 1 结束或进 `/af-sol` 前须落盘判定：`跳过 | 增量 | 全量`。

| 场景 | 动作 |
|------|------|
| 建议跳过 · user 已确认 | 总控写 `flow.yaml` model skip+reason → `AF_STEP=af-sol` |
| 建议跳过 · `ai` 自检齐 | 写 skip+reason → **同条进** `/af-sol` |
| 建议跳过 · user 未确认 | AskQuestion 建模判定 → **停** |
| 增量 | 只更新受影响的 model 文件 |
| 全量 | 完整流程 |
| 首个 REQ、尚无 model/ | **全量** |

**跳过主路径（有 flow.yaml）**：
1. `AF_STEP=af-mod`，对照该步 `criteria`
2. 可跳 → 写 `flow.yaml`：`skip: true` + `reason`
3. `advanceStep(projectRoot, 'af-sol')`（或成对改 AF_STEP+AF_PHASE）
4. 通常不派 model 角色；同条进 `/af-sol`

**必须进 model/（任一命中）**：新实体/聚合根/值对象；关系变；新规则/状态机；持久化结构变；首个 REQ 且无 model/；greenfield 有 DB；model/README 仍为草稿。

**目录（三层）**：
```
atlas/model/
├── README.md
├── conceptual/   # entity-relations.md · domain-rules.md
├── entities/{EntityName}.md
└── physical/schema.md
```

**执行**：总控判定 → 跳过路径或派 role-model → `mod-confirm` 绿 → user 确认停 / ai 连做 → 标 README 已确认。

**产出**：`atlas/model/**`（与 REQ 分文件）；本阶段只答「业务世界是什么」，不写 API、不拆开发任务。


## 何时执行本阶段

场景与动作表、跳过主路径 4 步 → [Agent 摘要](#agent-摘要)（SSOT）。

阶段 1 结束或进 `/af-sol` 前须落盘判定：`跳过 | 增量 | 全量`。`user` 问人；`ai` 自检齐→同条进 sol。须先判定再进 sol，避免无判定静默跳过。

## 建模按需判定

总控先按条件表**建议**「跳过 / 增量 / 全量」，再按下方规则问人或落盘；由总控派 Model Writer 执行。跳过须写 flow skip+reason，不自行静默跳过。
...
- **`ai` · 跳过**（四项自检 ✅ + 覆盖依据已校验）→ 总控写 `flow.yaml` `model.skip`+`reason` → `AF_STEP=af-sol` → 同条进 sol。首行：`建模跳过：同条进 sol`
- **`user`**：原话未点明跳过 → AskQuestion → **停**：

```yaml
title: "建模判定确认"
questions:
  - id: af-modeling-action
    prompt: |
...
**跳过确认后必须落盘 flow skip**（须写 flow，不只口头跳过）。可选镜像 todo「建模判定」行（**legacy**，有 flow 时以 flow 为准）：
...
- **greenfield 且有 DB/持久化**（须先建模再 skip）
...
基于已确认 REQ，将业务世界建模为**可阅读的独立文档**，只写入 `atlas/model/`（与 REQ、方案分文件维护）。
...
> 实体、关系、规则、DDL 落在三层子目录，不平铺在 `model/` 根下。
> 不用 `model-overview.md` 单文件收敛所有实体。
...
- 草稿后须结束闸门（user 确认卡 / ai 结束处理），不静默标「已确认」
...
- 实体/关系/规则/DDL 落在三层子目录，不平铺在根下
- 不用 `model-overview.md` 单文件收敛所有实体
- 本阶段不写 API、功能清单、开发任务；与 REQ/solution 分文件
...
`README.md` 未标「已确认」且未走「跳过」判定 → 须完成本阶段或写 flow skip 后再进阶段 3。
