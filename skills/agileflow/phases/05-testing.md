...
2. **AC 验收归档**（逐 REQ）：同会话且 ③ 证据可解析 → **默认不复跑**，引用 `## 结果`/checkpoint 摘要更新 AC 状态并出报告；跨会话 / 证据缺失 / 用户点名全量 → 复跑 `test/ac/` 后再归档 → REQ 已实现
...
> **本闸门 = 阶段 4 可运行闸门的延续，不再全量重跑。硬默认：同会话复用 ③ 证据。**
> 阶段 4 每个 T 的③已跑过编译+启动+冒烟；本阶段只做**增量验证**或**跨会话全量重验**。

| 场景 | 入场门禁动作 |
|------|-------------|
| **同会话**内阶段 4③ 可运行闸门已过（todo checkpoint 有时间戳 **且** 各 T `## 结果` 含可解析编译/启/冒烟证据） | **增量模式（硬默认）**：只验本次变更模块的**跨 T 集成冒烟**（多 T 之间的接口联动）；**不**重跑已验证的单 T 编译/启动/单 T 冒烟 |
| **新会话**（不论时间） | **全量重验**：编译 → 启动 → 冒烟（现行规则） |
| 可运行闸门证据缺失 / 不可复现 / checkpoint 无时间戳 | **全量重验** |
...
| 场景 | AC 归档动作 |
|------|-------------|
| **同会话** + 该 REQ 相关 T 的③已勾 + `## 结果`/`checkpoint` 含 AC 单测 exit0/✅/PASS 摘要 | **默认不复跑** `test/ac/`：直接引用摘要更新 AC 状态列 → 出 `atlas/tests/REQ-XXX-验收报告.md`（报告须写 `证据来源：阶段4③复用`） |
| 跨会话 / ③ 证据缺失或不可解析 / 用户点名全量重验 | **复跑** `test/ac/` → 再更新状态与报告 |

**禁止**：
- 无 ③ 证据时跳过入场/复跑直接出报告
- 冒烟失败仍标 AC 验收归档 ✅
- 用 `test/unit` 绿代替集成冒烟
- 同会话有合法 ③ 证据却无故全量重跑单 T（浪费；除非用户点名）
...
2. 选(b)：跑编译+单元/验收测试，跳过启动+冒烟；在 tests/README.md 标注 `⚠️ 冒烟环境待复现·仅验编译+测试`
