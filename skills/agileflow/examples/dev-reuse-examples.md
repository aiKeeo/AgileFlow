# dev 构思 · 步骤拆解 + 接现有代码（详细例子）

> **主形态（标准 / 完整，含 AI 自主）**：**原子步骤表**——每 `#### S1…` 一个步骤 + 8 字段规格表（执行角色/触发条件/输入数据/处理逻辑含 if-else/调用依赖 `Service.method(params)`/异常处理 错误码+回滚/输出数据/状态变更）。  
> **精简档**：可缩成一行 **改**。  
> **规则**：每步能看出系统怎么走；调用依赖里点明继续走 / 在…上加 / 照… / 新写哪份代码。  
> `AF_DECIDE=ai` / `AF_FLOW=fast` **不减**拆解厚度——完整档算法/链路照样走原子步骤表；要加速走并发，见 [fast-means-parallel](fast-means-parallel.md)。  
...
| 档位 / 场景 | 步骤形态 |
|-------------|----------|
| **完整** / 多步算法·链路 / **AI 自主做复杂功能** | **原子步骤表**（`#### S1…` + 8 字段规格表） |
| **标准** · 多步 CRUD / 跨层 | **原子步骤表** 优先 |
...
| 字段 | 内容 |
|------|------|
| 执行角色 | 前端 → 后端 `XxxController.method` |
| 触发条件 | 用户点击… / 上一步完成 |
...
|------|------|------|-------------|------------------|
| **S1** | 确定预测针对哪个用户 | 获取用户身份 | token → `user_id` | 继续走 `JwtFilter` + `UserContext` — 本 T 不重写登录 |
| **S2** | 限定预测的时间范围 | 圈定时间窗口 | 近 7/30/90 天 → `start_date, end_date` | 默认 30 天；校验写在挂到 `DashboardController` 的新接口上 |
| **S3** | 拿到这段时间的净热量数据 | 查询日均净卡路里 | user_id + 区间 → `avg_daily_net_calorie` | 净卡=摄入−运动；复用饮食/运动汇总查询，不平行造统计 SQL |
| **S4** | 算出每天热量缺口驱动后续脂肪换算 | 计算热量缺口 | 日均净卡 + BMR → `daily_deficit` | BMR **读**目标模块已有计算，不复制公式；净卡>BMR→增重要在响应里表达 |
| **S5** | 把热量缺口换算成脂肪重量 | 换算脂肪重量 | daily_deficit → `estimated_daily_fat_loss_kg` | 在 `PredictionService` 内用常量 7700 kcal≈1kg + 注释依据 |
| **S6** | 由脂肪重量推算未来体重变化 | 推算未来走势 | 日均脂肪 × 天数 → `predicted_weight_loss_kg` | 输出 7/30/90 三档；目标体重读现有 `Goal` |
| **S7** | 给前端返回可展示的预测结果 | 组装响应 | → 预测减重 / 到目标日 / 置信度 | 样本<7 天「低」，≥30 天「高」；包装走现有 `Result` |
...
| # | ❌ | 问题 | ✅ |
|---|----|------|-----|
| 1 | 只有「实现预测接口」两句 | AI 自主时空跑 | 用 S1–S7 原子步骤表 |
...
| 6 | 调用依赖无 \`Service.method\` | 闸门不过 | 调用依赖字段带代码落点 |
...
3. 完整档 / 算法链路是否用了原子步骤表（未因 `ai` 压成两句）？
4. 是否已读本端写法锚点？
