...
> **任务只写根 todo**：`atlas/todo.md`；不用 `atlas/solution/todo.md`。
...
**入口**：`/af-sol` 或 flow 到方案步。

**四要素**：

| # | 要素 | 落盘 |
|---|------|------|
| 1 | 功能（含边界） | `features/F-xxx.md` — §边界 + 暴露面 |
| 2 | 契约/接口（按需） | `contracts/` — 暴露面规格；feature 只链 ID |
| 3 | 架构（全局一份） | `architecture.md` |
| 4 | 任务编排 | `atlas/todo.md`（根）+ 功能依赖表 |

**标准流程**：
0. 契约/决策权：无 env → 启动卡；委托 → `AF_DECIDE=ai` 可跳
1. 总控读已确认 REQ +（按需）model/ + UID
2. 派 **role-sol** 写 solution/ + T 头建议；`user` 栈未定 → AskQuestion → 停
3. 收 T 头 → **总控写入 `atlas/todo.md`（根）** → `sol-confirm` 绿 → `AF_PHASE=4`
4. 确认后标 README **已确认**、沉淀 humanTodo、更新驾驶舱

**硬规则摘要**：
- 每个 REQ ≥1 feature；每 feature 须有 `## 边界` 且含 `← REQ-`
- 任务只写根 `todo.md`：`### T-xxx` + ①②③；禁 `solution/todo.md`
- greenfield `/af-sol` 默认建 `code-patterns-*.md` 🌱
- README 未「已确认」或 todo 无三段式 → 不进阶段 4

**产出**：`features/`、`contracts/`（按需）、`architecture.md`、`atlas/todo.md`（根）。
...
2. **总控派 role-sol**（加载 [role-sol](../templates/role/role-sol.md)）写 solution/ + T 头建议
   - `user`：技术栈未定时先 AskQuestion → 停；再派 role-sol 补 architecture；**先落盘再确认** → [contract §4 行为矩阵](../templates/contract.md#4-停点总表)（技术栈行）
...
本阶段前置：产物齐、todo 已写、humanTodo 已沉淀、`atlas/README.md` 已更新。
...
| 标记 | 规则 | 说明 |
|------|------|------|
| 🔴 | **REQ→feature 映射** | 每个 REQ 至少 1 feature；每个 feature 必须有 `## 边界` 且含 `← REQ-` 回溯；feature 不写 `## 联调卡` |
...
| 🟢 | **代码模式** | greenfield `/af-sol` 默认建 `solution/code-patterns-*.md` 🌱；**默认不建** `conventions/` |
| 🔴 | **Todo 格式** | 任务只写 **`atlas/todo.md`（根）**；必须 `### T-xxx` + ①②③（含 dev 路径）；不用扁平 `- [ ] T-001`；不用 `solution/todo.md` |
...
`README.md` 未「已确认」→ 不进阶段 4。
**todo 未过阶段 3 自检（无三段式）→ 不标方案已确认、不进阶段 4。**
