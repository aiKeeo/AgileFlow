...
> **`fast+ai` 连做 ≠ 单会话包办**：连做仍须**每阶段 / 每 T 真调用 Task**；连做的是停点，不是跳过 Subagent。  
> 首行：`📍 Agileflow | 模式：{AF_FLOW→连做/严谨} | 决策：{AF_DECIDE→AI全权/我来} | 阶段：{AF_PHASE} | …`（env 值须译成用户能懂的中文）
...
| 议题 | 裁决 |
|------|------|
| **首启契约（最高）** | 空着（`pending`）。没说清谁决策 → [contract 启动卡](templates/contract.md#71-流程启动卡)→停。明确委托（你定/别问我…）→ 可写 `AF_DECIDE=ai`。**只有 `ai` 全权**。**禁止**静默写 `fast+ai`。权威 → [contract](templates/contract.md) |
| **多 Agent / 总控（最高）** | 总控只路由、派 Subagent、**记派活台账**、跑 gate、更新状态。禁写产物正文。可写：`agileflow.env`·`todo.md`·`humanTodo.md`·驾驶舱·REQ 状态行。用户话术委托/接管/重选时**必须**改 `AF_FLOW`/`AF_DECIDE`。首启 `--bootstrap-scaffold` 落盘 `atlas/role/` + `humanTodo.md` + `atlas/agileflow-dispatch.json`。→ [orchestrator](templates/orchestrator.md) |
...
| **流程状态** | `atlas/agileflow.env`：`AF_PHASE`/`AF_FLOW`/`AF_DECIDE`/`AF_TIER=full`/`AF_STACK_SOURCE`；pending 或与产物不一致 → 闸门卡住 |
| **阶段结束** | `fast+ai`（**连做模式**）+闸门绿 → 摘要+连做；`strict+ai` → 审阅卡→停；`user` → 阶段闸门→停。细节 → [contract §4](templates/contract.md#4-停点总表) |
| **中途 AI 接管** | 「不想看了/后面都你定/剩下你来」等 → 立刻改 `AF_DECIDE=ai`（常+`fast`）。**审阅卡上**「不审了继续」= 只 skip 本卡。→ [contract §3](templates/contract.md#3-话术表必须看上下文) |
...
| ❌ | ✅ |
|----|----|
| 单会话包办正文 | 派 Subagent + 记台账 |
| `fast+ai` 连做跳过 Task | 连做仍每阶段/每 T 开 Subagent |
| 契约未确认静默 `fast+ai` | pending + 启动卡；明确委托才跳过 |
| 快速 → 薄写/跳闸门 | 快速=少停+并发；文档同质 |
| 你定 = 不写 atlas / 每阶段仍停 | 仍落盘；`fast+ai` 应连做 |
| `req:` → 目录 `atlas/req/` | 目录必须 `requirements/` |
| 先码后补 ① | 事后补写不勾① |
| 审阅卡上「不审了继续」当成全局接管 | 只 skip 本卡；自由文本「后面都你定」才接管 |
