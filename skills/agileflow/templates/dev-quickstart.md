...
**连续做 / 全部开发 / yes_all** = **先展开 TodoWrite**，再按清单逐条做（每条含可运行闸门）；**≠** 摘要 / 空壳 / 合并多 T / **从不编译启动** / **跳过 TodoWrite** / **扁平 todo**。快速模式同样遵守：无①禁写码、禁合并 T、禁跳过 `dev-step1-literal` / 可运行闸门。
...
| **步骤可执行** | **流程表**（`| **S1** |…|` 注意点含 \`落点\`）或每 `####` 含 **涉及改动**/**改**；标准≥2 / 完整≥3 / 精简≥1 步（legacy：`AF_TEMPLATE=no` 仍可用 用户/系统/改）。**完整档 / AI 自主复杂链路须流程表** | 按 exemplar / [dev-reuse-examples](../examples/dev-reuse-examples.md) 重写 |
| **链 sol** | BE 链 API-xxx；FE 链 UI-xxx（+UID）；dev **禁**字段映射表 | 补链 |
| **非事后补写** | 非事后补写；无 `顺序：⚠️` | 标 ⚠️；重写 |
| **反假标题** | 纯文本段名无 `##` → 不过；文档过短且无步骤（流程表或 ####）→ 不过 | 按 exemplar 重写整份 |
...
步骤：**流程表** S1…（动作 / 输入→输出 / 注意点含落点）优先；精简可用 `####` + `- **改**：` `` `Class.method()` ``。禁旧标题 `## 一、目标` / `## 五、可执行方案`。  
...
## 步骤          ← 流程表 S1…（完整/标准优先）或 ####+改（精简）；AC 只在摘要
## 结果          ← ③ 可运行证据
```
