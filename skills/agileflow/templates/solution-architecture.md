...
规则：每项 ≤4h；必须关联功能 ID；实现接口的任务须关联接口 ID；**每 T-xxx 必须含 ①②③ 三步子项与 dev 路径**；阶段 4 入口须 **TodoWrite 为每个 T 展开①②③三条**（见 [todo.md](todo.md) · [dev-quickstart](dev-quickstart.md)）。
...
| **人类驾驶舱** | 已更新 `atlas/README.md`（栈/现在/未决） | 缺驾驶舱 |
