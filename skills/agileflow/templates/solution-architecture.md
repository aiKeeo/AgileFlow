...
### 阶段 3 结束前自检（不过线禁止标方案已确认 / 禁止进阶段 4）

| # | 检查 | 不过线 |
|---|------|--------|
| S1 | 每个任务是 **`### T-xxx` 或 `#### T-xxx` 标题行**（可带 `[BE]`/`[FE]`）；**`## T-` 不算合法 T 头** | 只有 `- [ ] T-001 …` 扁平一行；或 `## T-001` |
| S2 | 每个 T 头下有 **① 构思落盘**（含 `atlas/dev/T-xxx` 路径）+ **②** + **③** 三行 | 缺任一步；无 dev 路径 |
| S2b | `todo`「开发任务」上方已含 **① 质量门槛**冻结表（Q1–Q7 + 机械 grep；**禁止改写成项目私货**） | 阶段 3 未写入 / 被改写成非 grep 门槛 |
| S3 | 禁止用「后端全部 / 前端全部」一条代替多 T | 合并多 T |
| S4 | 分组标题（如 `### 后端`）**不算** T 头；T 头必须形如 `### T-001` | 用分组冒充任务 |
