...
| **段标题** | 按档位齐全：精简=范围/做法/结果；标准·完整=+前置/必读/契约/AC | 补标题 |
| **必读（标准+）** | `## 必读` 含指向 `requirements/` 或 `solution/contracts/` 的链接；禁粘贴 API/UID 正文 | 补链接表 |
| **做法可执行** | 每个 `####` 含 `目的：` + 代码落点；标准≥2 / 完整≥3 / 精简≥1 步；禁空壳、「见 REQ」独撑 | 按 exemplar 重写 |
| **FE 布局映射** | FE+UI：`### 布局` 链 UID（或「布局差量」短 ASCII）+ `### 映射`（英文字段） | 补链/映射 |
| **非事后补写** | 非事后补写/非摘要；无 `顺序：⚠️` | 标 ⚠️；重写 |
...
> 权威 → [04-development §三档定义](../phases/04-development.md#三档定义) · [dev-rationale](dev-rationale.md)。精简=范围/做法/结果；标准·完整=+前置/必读/契约/AC。

做法：`#### 名 — 目的：…` + `` `Class.method` `` / `pages/` / `services/`。禁旧标题 `## 一、目标` / `## 五、可执行方案`。
...
|--------|----------|--------|
| **① 已勾** | 该 T 构思已勾① | 回 ① |
| **单任务约束** | 仍只做一个 T；Write ⊆ 当前 `## 做法` 步骤 | 收窄范围 |
| **读过写法锚点** | 已 Read **本端**写法锚点（`p1-frontend\|backend` 或 `code-patterns-*`）——**至少含资产索引**（[code-conventions](code-conventions.md)） | 先 Read |
| **复用就绪** | `## 契约` 中 `### 复用` 表写「复用」的路径须真用；禁止平行实现 | 改为复用 |
...
```
## 前置          ← 标准+：depends_on / 运行条件
## 必读          ← 标准+：只链 REQ/API/UID/model
## 范围          ← 目标 / 必须 / 不做
## 契约          ← 标准·完整档（精简可省）
  ### 布局       ← FE：链 UID（差量才画短 ASCII）
  ### 映射       ← FE+UI：英文字段 ↔ API
  ### 复用       ← 复用盘点
  ### 调用       ← FE：组件/服务调用链（按需）
## 做法          ← #### 名 — 目的：… + `Class.method`
## AC            ← 标准·完整档（精简可省；勿抄 Given/When）
## 结果          ← ③ 填可运行证据
```
