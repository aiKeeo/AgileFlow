...
> **质量铁律**：快速 / 严谨 / AI 自主 **同质**——禁止因「快速」薄写 F、契约、architecture、todo。要加速 → [并行](../phases/parallel-orchestration.md#快速模式推并发)。对照 → [fast-means-parallel](../examples/fast-means-parallel.md)
...
| **todo T 头** | `### T-xxx：[端] 简述 — F-xxx [档位]` + ①②③ 路径 | 扁平 `- [ ] 做登录`；多端塞一个 T |
...
> F **禁止**凭空编边界。AC 颗粒度与挂 T → [ac-guide.md](ac-guide.md)。
...
→ [UID-001](../../requirements/ui/UID-001-登录页.md)（线框唯一权威；本文件禁止粘贴整图）

> 仅当实现偏离 UID 时追加「布局差量」≤10 行 ASCII；默认不写。

## 组件树
（由 UID「关键组件与交互」展开为技术组件名，补状态/props）

## 字段绑定

> 行为见 REQ AC；接口形状见 API-001。

| 页面上 | 请求字段 | 接口 |
|--------|----------|------|
| 账号框 | `username` | API-001 POST body |
| 密码框 | `password` | API-001 POST body |
| 登录按钮 | — | 触发 POST API-001 |
