...
| **dev 文档** | 全档 **摘要+步骤+结果**；标准/完整步骤优先 **原子步骤表**（每 `#### S1…` 一个步骤 + 8 字段规格表：执行角色/触发条件/输入数据/处理逻辑含 if-else/调用依赖 `Service.method(params)`/异常处理 错误码+回滚/输出数据/状态变更）；精简可用一行 **改**；摘要须 **本T/做/不做/上游/AC**（**做**含接法）。`AF_DECIDE=ai` **不减**完整档步骤拆解。完整另字面量严检 |
| **快速 vs 严谨** | **只控摩擦与并发节奏**：`user_decide` 停点合并、`fast+ai` 免审阅卡、阶段 4 **推并发作业**（见 [parallel](phases/parallel-orchestration.md#快速模式推并发)）。**不**控 sol/dev **内容质量**——快速与严谨的 F/契约/architecture/todo/dev 颗粒度**必须同档**；**不**改 todo ①②③；**不**把 AI自主改成逐步追问（严谨+AI = 审阅闸门卡，不是更厚文档）。model 产物结构统一。<br><br>**硬边界**：**快 ≠ 薄文档**。禁止用「快速」压缩/模糊写 sol·dev；要加速 → **并发作业**（并行启动卡 + ≤3 T/批），不是少写步骤、不是「待补齐」。精简档仍是**风险分档**（与快速/严谨正交），仍须摘要五 bullet + 步骤落点可执行 |
...
| 空壳步骤（无涉及改动/无代码落点） | 摘要+原子步骤表（标准/完整）或方法级伪代码步骤（legacy 仍用用户/系统/改）（格式权威 → [dev-quickstart §构思闸门](templates/dev-quickstart.md#构思闸门勾-①-前)） |
...
| 严谨+AI自主仍逐步澄清/确认 | 落盘 → 审阅闸门；严谨多审阅卡，**不加厚** sol/dev（与快速同质） |
| 为求快把 F/dev 写成「做登录相关」 | **颗粒度铁律**：边界可对 AC、步骤有落点；例子 → [fast-means-parallel](examples/fast-means-parallel.md) |
...
| AI 的借口 | 真实情况 |
|-----------|----------|
| 「用户说快，所以我跳了闸门 / 薄写 sol·dev」 | **快速 ≠ 跳阶段 ≠ 薄文档**；仍走五步、sol/dev 同质；要加速 → **推并发**（并行启动卡） |
...
默认串行；显式并行/要快，或**快速模式推并发** + 并行启动卡。→ [parallel-orchestration](phases/parallel-orchestration.md) · 对照 [fast-means-parallel](examples/fast-means-parallel.md)
...
| 说法 | 意思 |
|------|------|
| 委托交付 / fast+ai | 用户**不想自己决策**：AI 推断并落盘，少停少卡；≠ 跳阶段、≠ 薄 sol/dev、≠ 先码后补；要加速 → 并发 |
| 快=并发 | 快速模式正当加速手段=并行作业；sol/dev 与严谨同质 → [fast-means-parallel](examples/fast-means-parallel.md) |
