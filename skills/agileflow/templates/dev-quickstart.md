...
| **段标题** | 按档位齐全：精简=范围/做法/结果；标准·完整=+契约/AC | 补标题 |
| **做法可执行** | `## 做法` 下 `####` 步骤含代码落点：`` `Class.method` `` / `` `func_name()` `` / `` `pages/` `` / `` `services/` ``；禁空壳、「见 REQ」独撑 | 重写做法 |
| **FE 布局映射** | FE+UI：`### 布局`（ASCII 线条图）+ `### 映射`（英文字段） | 补布局/映射 |
...
→ [dev-rationale](dev-rationale.md) · 完整档跑 `--gate dev-step1-literal --dev-file …`

| 档 | 必填 |
|----|------|
| 精简 | `## 范围` `## 做法` `## 结果` |
| 标准·完整 | + `## 契约` `## AC`；FE：`### 布局`+线框、`### 映射` |

做法内 `####` 步骤须含 `` `Class.method` `` / `pages/` / `services/`。禁旧标题 `## 一、目标` / `## 五、可执行方案`。
...
## 范围          ← 目标 / 必须 / 不做
## 契约          ← 标准·完整档（精简可省）
  ### 布局       ← FE+UI：ASCII 线框图（禁纯表）
  ### 映射       ← FE+UI：英文字段 ↔ API
  ### 复用       ← 复用盘点
  ### 调用       ← FE：组件/服务调用链（按需）
## 做法          ← #### 步骤含 `Class.method` / pages
## AC            ← 标准·完整档（精简可省）
## 结果          ← ③ 填可运行证据
```
