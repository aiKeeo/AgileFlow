...
AI 会：写 `REQ-001` → 跑 `req-confirm` → 写 `sol: architecture + F-001 + contracts/API-001` → 跑 `sol-confirm` → 拆 `T-001/T-002` → 每 T 写 `dev/T-*.md` + 跑 `dev-step1-literal` + 写码 + 跑可运行闸门 → 出验收报告。快速模式只减少 AskQuestion 停点，不跳过 `atlas/` 落盘与闸门。
...
支付/并发/幂等触发「完整档」：REQ AC 更细、方案含异常与幂等设计、dev 用流程表+字面量严检、TDD 强制。每阶段须用户确认或审阅闸门通过。
...
> ⚠️ **快速模式警示**：快速 ≠ 跳过阶段。快速模式仍须 `req → sol → dev` 按序落盘，每个开发任务必须有 `①构思 / ②写码 / ③AC`，勾③前必须过可运行闸门。省的是「问你的次数」，不是「文档质量」。
...
| **快速模式** | 少问、快走、文档精简，但**不减格式、不减闸门、不减可运行证据**。适合 Demo/CRUD/内部工具 |
| **严谨模式** | 多确认、厚文档、TDD 强制。适合支付/权限/核心业务 |
