# 阶段决策委派（AI 自主 vs 用户决策）

> **核心**：每阶段入口可问用户——**本阶段谁来做决策**？  
> - **用户决策**：逐步 AskQuestion，用户选完再落盘  
> - **AI 自主**：Agent 自行推断并落盘，**产出必须可审阅**；审阅**可跳过**  
> 权威搭配：[flow-modes.md](flow-modes.md)（快速/严谨）· [askquestion-gate.md](askquestion-gate.md)

---

## 两维模型

| 维度 | 选项 | 控制什么 |
|------|------|----------|
| **流程模式** | 快速 / 严谨 | 文档厚度、合并闸门次数 |
| **决策权** | 用户决策 / **AI 自主** | 阶段内**选型/澄清**要不要问用户 |

**正交**：快速 + AI 自主 = 最少交互；严谨 + 用户决策 = 最多把关。

---

## 阶段入口：决策权 AskQuestion

进入阶段 0/1/2/3/4 **落盘前**：

| 条件 | 动作 |
|------|------|
| `todo` 已标 **全局 AI自主**（且用户未说「这阶段我来」） | **跳过**本卡 → 直接按 ai_decide 落盘 → 审阅闸门 |
| 用户本轮已说「你定 / 后面都你定」 | 按话术表设 ai_decide / global，**可跳过**本卡 |
| 决策委派 **未设置** | **必须**弹决策权卡（可与模式判定合并为 1 卡多题） |

```yaml
title: "{阶段名} - 决策方式"
questions:
  - id: stage_decision_mode
    prompt: "【{阶段名}】本阶段关键选型/澄清，你想？"
    options:
      - id: user_decide
        label: "我来决策（逐步问我，写完再确认）"
      - id: ai_decide
        label: "AI 自行决策（你写完落盘，我看不看都行）"
      - id: ai_decide_global
        label: "AI 自行决策，且后续阶段也默认 AI（可在 todo 改回）"
```

| 选项 | 行为 |
|------|------|
| **user_decide** | 走该阶段原有 AskQuestion 链（需求卡、技术栈等） |
| **ai_decide** | **仅本阶段**跳过阶段内决策卡；Agent 推断 → 落盘 → [审阅闸门](#审阅闸门ai-自主专属) |
| **ai_decide_global** | 本阶段 AI 自主 + 在 `atlas/todo.md` 写 `决策委派：AI自主（自 {阶段} 起）`；后续阶段入口**默认跳过决策权卡**（用户可说「这阶段我来」覆盖） |

**调用后**：`user_decide` → 按原流程停止等用户；`ai_decide*` → **下条回复必须落盘**（禁止只寒暄）。

### 全局委派记录（todo.md）

```markdown
## 决策委派
- 全局：AI自主 | 用户决策 | 未设置
- 自阶段：{req|mod|sol|dev} 起
- 覆盖：用户说「这阶段我来决策」→ 仅当阶段改回 user_decide
```

---

## AI 自主：阶段内跳过什么

| 阶段 | AI 自主时跳过 | AI 必须自行决定并落盘 | 仍不可跳过 |
|------|--------------|----------------------|------------|
| **0 init** | init 确认前的反复澄清 | 扫描结论、分层、实体用途 | 落盘自检；审阅闸门 |
| **1 req** | 需求澄清卡、草稿确认卡 | 用户/平台/MVP/功能拆分/UID 范围 | REQ+AC+README；**AI 决策记录** |
| **2 mod** | 建模确认卡 | 实体/关系/规则/表结构（或 model-overview） | model/ 落盘 |
| **3 sol** | 技术栈卡、方案确认卡 | 技术栈、features、contracts、architecture、todo | solution/ 落盘 |
| **4 dev** | 构思审阅（可选） | 完整模板（禁摘要/空壳）；逐 T 过闸门 A/B | ①②③；禁批量① |

**禁止 AI 自主时跳过**：`atlas/` 落盘、dev ① 构思文件、AC 只引用 REQ、humanTodo 沉淀（密钥/参考稿仍须列出）。

---

## AI 决策记录（落盘必填）

AI 自主完成阶段后，在该阶段 **README.md 或主产出文末**追加：

```markdown
## AI 决策记录（本阶段 Agent 自行决定，供审阅）

| 决策点 | 选择 | 依据 |
|--------|------|------|
| 技术栈 | {按 REQ/仓库选定} | {一句依据} |
| MVP 范围 | {本阶段纳入的功能} | 用户原话优先级 |

> 审阅时重点看此表；有异议选审阅闸门「我要调整」。
```

---

## 审阅闸门（AI 自主专属）

AI 自主落盘完成后，**用此卡替代**「草稿确认 + 阶段闸门」两步（快速/严谨均适用）：

```yaml
title: "{阶段名} - 产出审阅（可选）"
questions:
  - id: review_action
    prompt: |
      本阶段已写入：
      - {文件列表摘要}
      
      AI 决策摘要：{3～5 条 bullet}
      
      你可以打开 atlas/ 细看；也可以不审直接继续。
    options:
      - id: confirm_continue
        label: "看过了，确认并继续下一阶段"
      - id: skip_review_continue
        label: "不审了，信任 AI，直接继续下一阶段"
      - id: need_changes
        label: "我要调整（下轮改文档）"
      - id: pause
        label: "暂停，我先自己看文件"
```

| 选项 | Agent 下条回复 |
|------|----------------|
| confirm_continue / skip_review_continue | 标本阶段 **已确认** → **下条回复**进入下一阶段并落盘 |
| need_changes | 按用户文字修改本阶段产出 → 再发审阅闸门 |
| pause | 仅更新 todo，**不写**下一阶段 |

**审阅可跳过**：`skip_review_continue` 合法；标 README **已确认** 等同用户确认。

**仍禁止**：选「继续」后只寒暄不写下一阶段文件。

---

## 与用户话术对照

| 用户说 | 解析 |
|--------|------|
| 「这阶段你定 / 不用问我了 / 全自动」 | 当阶段 → **ai_decide** |
| 「后面都你定」 | **ai_decide_global** |
| 「这阶段我来」 | 覆盖全局 → **user_decide** |
| 「不审了继续 / 跳过审阅」 | 审阅闸门 → **skip_review_continue** |
| 「我自己看，暂停」 | **pause** |

---

## 交互次数对比（阶段 1 例）

| 路径 | 停几次 |
|------|--------|
| 严谨 + 用户决策 | 3（需求卡 → 确认 → 闸门） |
| 快速 + 用户决策 | 2（需求卡或跳过 → 合并确认闸门） |
| 快速 + **AI 自主** | **1～2**（无全局委派时决策权卡 → 审阅闸门；已全局 AI自主则**仅审阅闸门**） |
| AI 自主 + skip_review | 用户选「不审继续」→ 下条直接写下一阶段 |

---

## 正误示例

**✅ AI 自主·req**：入口选 AI 自主 → 写 REQ+AC+UID+决策记录 → 审阅闸门 → 用户选「不审继续」→ 下条写 solution/

**✅ 全局 AI**：todo 标 AI自主 → 阶段 3 跳过技术栈卡 → 写 architecture+todo+决策记录 → 审阅闸门

**❌ AI 自主但不写 atlas/**（必须落盘）

**❌ AI 自主跳过 dev ① 构思**（只跳过「问用户」，不跳过构思文件）

**❌ 用户选 pause 后 Agent 擅自写下一阶段**
