...
> **阶段 4 ③ 对照 REQ 验收 AC**：**② 写码完成后**，按 dev **## AC** 引用的 REQ AC 在 `test/ac/` 写验收测试并跑全绿（细则 [04-development.md](../phases/04-development.md)）。
...
| 顺序 | ① 按 **风险档位**落盘 → 构思闸门 → 勾① → 写码闸门 → ② → **可运行闸门** → ③。段结构看 todo 的 `[精简\|标准\|完整]`（精简=范围/做法/结果；标准·完整=+契约+AC），**与快速/严谨无关**；禁止摘要版 |
...
- **测试入场门禁**：编译构建 → 启动探针 → 功能冒烟；不过禁止 AC 验收归档（见 [测试流水线](l1-l5-pipeline.md#测试入场门禁) · [05-testing](../phases/05-testing.md#测试入场门禁强制)）  
- **AC 验收归档**：复跑 `test/ac/` → AC 状态 ✅ → `atlas/tests/REQ-XXX-验收报告.md`  
