...
**UI-xxx 来源**：读 REQ 阶段 [UID](../../requirements/ui/) → 转写为技术契约（路由、组件、状态、绑定的 API-xxx）。**UID 有的结构、交互与布局线条图必须保留**；样式以用户 sol 阶段确认为准。
...
> 须含 ASCII 线框；禁止只写「见 UID」。可微调控件名以贴合组件库，但区块关系不得丢。
